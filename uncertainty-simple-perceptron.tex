...
Following the introduction of the simple perceptron by Rosenblatt in 1957 \textbf{[b7]}, Cover formalized the principle that \textit{“complex pattern classification problems, cast in a high-dimensional space nonlinearly...”} \textbf{[b2]}. This observation suggests that nonlinear projections into higher-dimensional spaces increase the probability of achieving linear separability, enabling simple models to perform effectively in complex pattern recognition tasks.

\section{Background}

Linear models such as the simple perceptron \textbf{[b7]} or linear regression \textbf{[b4]} share the property of computing a linear combination of input features. In classification settings, this linear response is typically mapped to a binary window decision.

These models operate under the \textit{“formulation of the "general linear combination”} \textbf{[b4]}. In the case of the simple perceptron, the model computes the dot product between the input feature vector and the corresponding vector, followed by the addition of bias term:

$$
z = \sum_{i=1}^{n} w_i x_i + b
$$

The resulting scalar value is then passed through an activation function. Each model employs a different activation depending on its objectives; for example, the simple perceptron uses a step function to produce binary output \textbf{[b7]}.
...
In the mid-20th century, something similar to a \textit{“machine learning winter”} occurred. Models such as the simple perceptron lacked the geometric capacity to solve problems such as XOR \textbf{[b6]}, leading to the dropout of these models and their use was only retained as a foundation for subsequent generations of models.

Although the non-linear limitations of the simple perceptron with respect to the XOR problem led to the prioritization of multi-layer architectures \textbf{[b6]}, current energy constraints in edge computing warrant a re-evaluation of linear models when coupled with robust feature engineering.
...
Current machine learning research often prioritizes representation learning architectures (e.g., LSTMs or Transformers). However, for non-stationary time-series, the computational burden of learning temporal dependencies can in many cases be replaced by trigonometric cyclic encoding and stochastic oscillators \textbf{[b12]}. Shifting from “learning the structure” to explicitly “encoding the structure” allows linear models to remain competitive in high-entropy settings, especially when selective prediction and reliability-oriented performance metrics are used.
...
The introduction of the region $[-\epsilon, \epsilon]$ transforms the usual decision into a geometric margin. Following the principles established by Cortes and Vapnik, maximizing the distance of data points from the separating hyperplane improves generalization \textbf{[b9]}. This approach is consistent with modern selective classification strategies that prioritize precision over coverage \textbf{[b13]}.
...
To compute the uncertainty margin of the model, the standard deviation ($\sigma$) is proposed, which Karl Pearson defined as \textit{"...a natural measure of the "scatter" of observations."} \textbf{[b10]}. 
...
During each learning epoch, the model updates its parameters using the Perceptron learning rule \textbf{[b7]}. The weight adjustment is calculated as the product of the learning rate ($\eta$) and the prediction error, defined as the difference between the true label ($y$) and the predicted output of the perceptron ($\hat{y}$), which is obtained from the activation step function (0 or 1).
...
With the split data set, the model was trained as usual, following the Rosenblatt approach \textbf{[b7]}. After completion of the full training phase, uncertainty was added during post-training testing, and the model was run across the entire dataset using the same sliding-window approach.
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c}
        ...
        \hline
        Type I & $\approx3.34$ & $\approx40000$\\
        Type II & $\approx9.44$ & $\approx110000$\\
        Type III & $\approx6.14$ & $\approx80000$\\
        Type IV & $\approx3.74$ & $\approx37000$\\
        \hline
        \multicolumn{3}{p{0.9\linewidth}}{\centering
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c}
        ...
        Selected Coverage & $79.7\%$ & $\approx89.3\%$ Coverage\\
        Model Dimensions & 1080 & N/A\\
        Training Time & $\approx12$ Seconds & N/A\\
        \hline
        \multicolumn{3}{p{0.9\linewidth}}{\centering
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c}
        ...
        Training Time & $\approx17$ Seconds & N/A\\
        \hline
        \multicolumn{3}{p{0.9\linewidth}}{\centering
...
The selected models for this comparison—Bonsai \textbf{[b11]}, FastGRNN \textbf{[b12]}, and ProtoNN \textbf{[b11]}—were chosen based on their established performance in TinyML benchmarks.
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c}
        ...
        \hline
        Uncert. Simple-percept. & $\mathcal{O}(e * n * d)$ & $\mathcal{O}(d)$\\
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c}
        ...
        \hline
        Uncert. Simple-percept. & $\approx1$ KB & $\approx15$ Micro-sec\\
        Bonsai & $\approx4.5$ KB & $\approx13$ Micro-sec\\
        FastGRNN & $\approx47$ KB & $\approx2$ Micro-sec\\
        ProtoNN & $\approx16$ KB & $\approx3$ Micro-sec\\
        Linear SVM & $\approx2$ KB & $\approx15$ Micro-sec\\
        Standard LSTM & $>500$ KB & Various-sec\\
        \hline
        \multicolumn{3}{p{0.9\linewidth}}{\centering
        Benchmarks were conducted on a constrained virtualized environment (single-core 64-bit CPU, 1 MB RAM limit) to simulate resource scarcity in embedded systems.
        }
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c}
        ...
        \hline
        Uncert. Simple-percept. & $\approx69.4\%$ & $\approx89.3\%:\approx79.7\%$\\
