...
\begin{table}[H]
    ...
    \begin{tabular}{c c}
        ...
        Dataset was split into $73\%$ for training, corresponding to $27\%$ for testing.
        }
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{c c c}
        ...
        \hline
        \multicolumn{3}{p{0.9\linewidth}}{\centering
        By rejecting uncertain samples during the transient degradation phase, the model achieves near-perfect accuracy in confirmed states, acting as a robust low-power early warning system.$\mathbf{79.3\%\text{ Cov.} \ \& \ 93.7\%\text{ Acc.}}$
        }
    \end{tabular}
\end{table}
...
The hardware used during training consisted of a single 64-bit CPU core and $\approx 1$ MB of RAM. During the inference phase, the model used only $\approx 1$ KB of memory, since it did not need to load the data set into memory.
